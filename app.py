...
# Function to convert Knots to contiimport datetime
import math
import os
import numpy as np
import pandas as pd
...
if os.path.exists(CSV_FILE):
    ...
    if not df.empty and "timestamp" in df.columns:
        ...
        # 3. Gust Trace in Stretched Scale (Dotted Black Outline)
    ...
    st.info("No data file found yet.")nuous Beaufort Scale Force
def knots_to_bft(knots):
    if isinstance(knots, pd.Series):
        s = pd.to_numeric(knots, errors="coerce").clip(lower=0)
        return np.power(s / 1.625, 2.0 / 3.0)
