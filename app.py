...
# Continuous Beaufort Color Scales
...
st.caption("Real-time weather station monitor with **Stretched Beaufort Scale & Wind Vectors** (*Optimized for high performance*).")

# --- Optimized Data Loader with 60s Cache ---
...
if df is not None and not df.empty:
    ...
    # --- Fast Vectorized Label Computation ---
    ...
    if valid_indices:
        ...
        last_g_val = -999.0
        last_g_idx = -999
        fallback_step = max(4, len(valid_indices) // 25)
        ...
        for idx in valid_indices:
            ...
            if abs(s_val - last_s_val) >= 1.0 or (idx - last_s_idx) >= fallback_step:
                speed_labels[idx] = f"{s_val:.1f}"
                labeled_speed_points.append({
                    "timestamp": t_arr[idx],
                    "velocita_plot_y": y_arr[idx],
                    "direzione_deg": deg_arr[idx]
                })
                last_s_val = s_val
                last_s_idx = idx
    ...
    # --- Lightweight Adaptive Gradient (Limits total bars to < 200 to maximize DOM speed) ---
    target_points = 180
    total_len = len(df_plot)
    resample_freq = max(1, total_len // target_points)
    df_gradient_fill = df_plot.iloc[::resample_freq].copy()
    
    # Calculate responsive bar width in milliseconds
    ...
    fig.add_trace(go.Bar(
        ...
        marker=dict(
            ...
            cmin=0, cmax=8, line=dict(width=0)
        ),
        width=bar_width_ms,
        hoverinfo="skip",
        showlegend=False,
    ...
    # Subplot 1: Vector Arrows Under Speed Numbers
    mini_arrow_len = 16
    ...
    # Subplot 2: Adaptive Compass Arrows
    df_for_arrows = df_filtered.copy().reset_index(drop=True)
    steady_step = max(4, len(df_for_arrows) // 25)
    selected_indices = []
